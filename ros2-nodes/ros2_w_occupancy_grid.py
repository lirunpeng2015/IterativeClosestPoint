...
class PersistentMapBuilder(Node):
    def __init__(self):
        ...
        self.max_num_of_stored_scans = 20 # Optimized number of scans for ICP to be fast enough
        self.prev_scans_transformed = [] # List of previous scans
        self.prev_n_points = None # Array of points from previous scans
        self.grid_publish_counter = 0

        # Occupancy grid initialization and setup
        self.grid_resolution = 0.005 # meters per cell
        self.grid_width = 10000 # in cells
        self.grid_height = 10000
        ...
        self.occupancy_grid.info.origin.orientation.w = 1.0  # No rotation

        self.occupancy_grid.data = [-1] * (self.grid_width * self.grid_height) # Initialize with -1 (unknown)
    ...
    def laser_callback(self, msg):
        ...
        # Subsequent scans
            ...
            new_points_transformed = (self.global_rotation @ new_points.T).T + self.global_translation.T
            self.update_occupancy_grid(new_points_transformed)
            ...
            # Subsequent scans
                ...
                self.prev_scans_transformed.pop(0)
                self.prev_scans_transformed.append(new_points_transformed)
        ...
        if self.grid_publish_counter <= 10:
            self.grid_publish_counter += 1
        else: 
            self.grid_publish_counter = 0
    ...
    def update_occupancy_grid(self, transformed_points):
    ...
    def icp(self, reference_points, source_points, max_iterations=10, tolerance=1e-4):
        R = np.eye(2)
        t = np.zeros((2, 1))
